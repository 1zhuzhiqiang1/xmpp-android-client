...
public class XmppTool {
    ...
    private static final String HOST = "47.93.197.48";
    ...
    private XmppTool(final Context context) {
        ...
        try {
            ...
            xmppConnection.addConnectionListener(new ConnectionListener() {
                ...
                public void reconnectionFailed(Exception arg0) {
                    // TODO Auto-generated method stub
                    SLog.i(tag, "重连失败");
                    Toast.makeText(context, "连接服务器失败，请检查网络连接", Toast.LENGTH_SHORT).show();
                }
    ...
    public boolean login(String name, String pwd, Context context) {
        try {
            ...
            xmppConnection.login(name.toLowerCase(), pwd);
            // getMessage();//获取离线消息
            int status = SharedPreferencesUtil.getInt(context, "status", name + "status");
            setUserState(status);//设置状态,默认为在线状态
            return true;
        } catch (XMPPException e) {
            SLog.e(tag, Log.getStackTraceString(e));
        }
...
//                break;
    ...
    public void setUserState(ImageView iv, ImageView iv_me, Context context, String name) {
